...
pub struct RemoteTeleopProcess2(Arc<Mutex<HashMap<String, (CommandChild, Arc<AtomicBool>, String)>>>);

pub struct RemoteTeleopService2;

impl RemoteTeleopService2 {
    pub fn init_remote_teleop() -> RemoteTeleopProcess2 {
        RemoteTeleopProcess2(Arc::new(Mutex::new(HashMap::new())))
    }

    pub async fn start_teleop(
        app_handle: AppHandle,
        db_connection: DatabaseConnection,
        state: &RemoteTeleopProcess2,
    ...
    pub fn stop_teleop(
            ...
            state.0.lock().unwrap().remove(&nickname)
        {
            ...
            ProcessService::on_process_shutdown(child.pid(), db_connection, command_log_id);
