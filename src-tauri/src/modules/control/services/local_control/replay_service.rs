use crate::modules::control::controllers::local_control::replay_controller::ReplayConfig;
...
pub struct ReplayProcess(Arc<Mutex<HashMap<String, (Child, Arc<AtomicBool>, String)>>>);

pub struct ReplayService;

impl ReplayService {
    pub fn init_replay() -> ReplayProcess {
        ReplayProcess(Arc::new(Mutex::new(HashMap::new())))
    }
    ...
    pub async fn start_replay(
        ...
        let command_parts = build_command_parts(&config, &robot_type);
        let command_parts_for_log = build_command_parts_for_log(&config, &robot_type);
...
fn build_command_parts(config: &ReplayConfig, robot_type: &str) -> Vec<String> {
    let mut parts = vec!["python".to_string(), "src/lerobot/scripts/lerobot_replay.py".to_string()];
...
fn build_command_parts_for_log(config: &ReplayConfig, robot_type: &str) -> Vec<String> {
    let mut parts = vec!["python".to_string(), "src/lerobot/scripts/lerobot_replay.py".to_string()];
