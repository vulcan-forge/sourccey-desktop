...
impl AccessPointService {
    ...
    pub async fn set_access_point(
        ...
        // Build the script path - scripts are in scripts/connection/ relative to lerobot-vulcan dir
        let script_path = format!("{}/scripts/connection/set_access_point.py", working_dir_str);
                ...
                if let Ok(json_output) = serde_json::from_str::<serde_json::Value>(&output) {
                    ...
                    if status == "SUCCESS" {
                        let ip_address = json_output
                            .get("ip_address")
                            .and_then(|v| v.as_str())
                            .unwrap_or("192.168.4.1");
                        let message = json_output
                            .get("message")
                            .and_then(|v| v.as_str())
                            .unwrap_or("Access point activated successfully");

                        let _ = app_handle.emit(
                            "access-point-set-success",
                            serde_json::json!({
                                "ssid": ssid,
                        ...
                        println!("[AccessPoint] Access point mode activated successfully");
                        Ok(format!(
                            "Access point '{}' activated successfully. IP: {}",
                            ssid, ip_address
                        ))
                    } else {
