...
Launching the app in kiosk mode is optional and opt-in.
"""
...
class DevKioskSetupScript:
    ...
    def detect_project_root(self) -> bool:
        ...
        if not (self.project_root / "package.json").exists():
            self.print_error("Could not detect project root from script location")
    ...
    def run_kiosk_dev(self) -> bool:
        ...
        try:
            ...
            self.print_error(f"Failed to run kiosk dev: {exc}")
            return False
    ...
    def print_summary(self):
        ...
        if not self.errors:
            ...
            print()
            self.print_status("You can now start kiosk mode with: bun tauri dev -- -- --kiosk")
            print()
    ...
    def run(self, use_https: bool = False, launch: bool = False) -> bool:
        ...
        if not self.ensure_uv():
            self.print_error("uv is required for Sourccey kiosk setup.")
        ...
        if not self.setup_git_submodules(use_https=use_https):
            ...
            if not use_https:
                ...
                self.print_error("")
                self.print_error(
                    f"  {self.current_python_command()} setup/kiosk/setup_dev.py --use-https"
                )
                self.print_error("")
            else:
        ...
        if len(self.errors) == 0 and launch:
...
def main():
    ...
    parser.add_argument(
        "--launch",
        action="store_true",
        help="Launch kiosk dev mode after setup completes",
    )
    args = parser.parse_args()

    setup = DevKioskSetupScript()
    success = setup.run(use_https=args.use_https, launch=args.launch)
