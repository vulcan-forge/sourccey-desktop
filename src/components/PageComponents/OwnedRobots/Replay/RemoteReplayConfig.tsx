...
import { setRemoteReplayConfig, useGetRemoteReplayConfig } from '@/hooks/Components/OwnedRobots/remote-config.hook';

export const RemoteReplayConfig = ({ ownedRobot }: { ownedRobot: any }) => {
    const nickname = ownedRobot?.nickname ?? '';
    const { data: config }: any = useGetRemoteReplayConfig(nickname as string);

    const handleConfigChange = (field: keyof RemoteReplayDatasetConfig, value: string | number) => {
        setRemoteReplayConfig(nickname as string, { ...config, [field]: value });
    };
            ...
            <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input
                        type="number"
                        min="0"
                        value={config?.episode ?? 1}
                        onChange={(e) => handleConfigChange('episode', parseInt(e.target.value) || 1)}
                        className="border-slate-625 bg-slate-725 w-full rounded border px-2 py-1 text-sm text-white placeholder-slate-400 transition-all duration-200 focus:ring-1 focus:ring-yellow-500 focus:outline-none"
                        placeholder="1"
                    />
                </div>
...
export interface RemoteReplayConfig {
    nickname: string;
    remote_ip: string;
    dataset: RemoteReplayDatasetConfig;
}

export interface RemoteReplayDatasetConfig {
    dataset: string;
    episode: number;
    fps: number;
}
