import { setRecordConfig, useGetRecordConfig } from '@/hooks/Components/OwnedRobots/config.hook';
import { FaVideo } from 'react-icons/fa';
import type { CameraConfig } from '@/components/PageComponents/OwnedRobots/Teleop/TeleopAction';

export const RecordConfig = ({ ownedRobot }: { ownedRobot: any }) => {
    const nickname = ownedRobot?.nickname ?? '';
    const { data: config }: any = useGetRecordConfig(nickname as string);

    const handleConfigChange = (field: keyof RecordDatasetConfig, value: string | number) => {
        setRecordConfig(nickname as string, { ...config, [field]: value });
    };
...
export interface RecordDatasetConfig {
    dataset: string;
    num_episodes: number;
    task: string;
