...
export const RobotKioskCalibration: React.FC<CalibrationSectionProps> = ({ nickname, robotType = 'sourccey', calibration }) => {
    ...
    const [showCalibrationSection, setShowCalibrationSection] = useState(false);
    ...
    return (
        <div className="rounded-xl border-2 border-slate-700 bg-slate-800 p-6 backdrop-blur-sm">
            ...
            >
                ...
                <button onClick={() => setShowCalibrationSection(!showCalibrationSection)} className="cursor-pointer">
                    {showCalibrationSection ? <FaChevronUp className="h-5 w-5 text-slate-400" /> : <FaChevronDown className="h-5 w-5 text-slate-400" />}
                </button>
            </div>
